...
class Index:
    ...
    def add_msg_and_index(self, m):
        self.add_msg(m)
        line_at = self.total_msgs - 1   # -1的目的是与self.msgs匹配
    ...
    def indexing(self, m, l):
        ...
        # ---- start your code ---- #
        mline = []
        mline.extend(m.split(' '))  # 将这一条message按照单词分在列表里

        for i in mline:    # 遍历这个列表，将里面每一个单词都作为key在index这个dictionary里添加一遍这个message的line number（value）
            try:
    ...
    def search(self, term):
        ...
        pass
        try:
            lines = self.index[term]
            for item in lines:
                tup = (item, self.msgs[item])
                if tup not in msgs:
                    msgs.append(tup)
            return msgs
        except:
...
class PIndex(Index):
    ...
    def get_poem(self, p):
        ...
        # ---- start your code ---- #
        pass
        start = self.msgs.index(self.int2roman[p]+'.')
        end = self.msgs.index(self.int2roman[p+1]+'.')
        for line in range(start, end):
            poem.append(self.msgs[line])
